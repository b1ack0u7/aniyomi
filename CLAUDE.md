...
**Strings:** edit only `i18n*/src/commonMain/moko-resources/base/{strings,plurals}*.xml`. Everything else is generated by Weblate and both CI and Spotless deliberately ignore non-`base` locale files. Use `MR.strings.*` for Mihon-inherited strings and `AYMR.strings.*` for Aniyomi-only ones, via `stringResource(...)` from `tachiyomi.core.common.i18n`.
