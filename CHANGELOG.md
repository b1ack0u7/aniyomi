...
- Fix release builds crashing when a source responds with zstd-compressed content, by keeping the zstd classes that okhttp-zstd resolves through JNI from being stripped by R8 ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix the scrollbar drawing over the toolbar while scrolling the anime entry screen, by insetting the fast scroller below the app bar as the manga screen already did ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix the anime entry screen ignoring selection mode: pressing back now clears the episode selection instead of leaving the screen, and the play button and pull-to-refresh hide and disable while episodes are selected, matching the manga screen ([@b1ack0u7](https://github.com/b1ack0u7))

### Other

- Rebuild an entry's selection against its full episode/chapter list instead of the filtered view, so selecting an item while a filter is active no longer narrows the backing list, on both sides ([@b1ack0u7](https://github.com/b1ack0u7))

## [v0.21.0] - 2026-08-08
### Added

- Add a "Similar Titles" row to anime and manga entry screens, combining recommendations from AniList, MyAnimeList and MangaUpdates with related titles from the entry's own source; can be turned off under Settings → Browse ([@b1ack0u7](https://github.com/b1ack0u7))
- Collapse chapter and episode lists to the first 10 entries, with a per-side toggle under Settings → Appearance → Title screens to always show the full list ([@b1ack0u7](https://github.com/b1ack0u7))

### Improved

- Complete the Spanish translation, filling in 46 strings and 10 plurals that had no `es` entry ([@b1ack0u7](https://github.com/b1ack0u7))
- Show library update progress in a bar across the library and updates tabs, with a summary message when the run ends ([@b1ack0u7](https://github.com/b1ack0u7))
- Stop the download queue from waking up constantly while idle, cutting its battery use ([@b1ack0u7](https://github.com/b1ack0u7))
- Speed up chapter downloads by scanning the chapter folder once instead of twice per page ([@b1ack0u7](https://github.com/b1ack0u7))
- Speed up queueing chapters and episodes by listing the download folder once instead of per entry ([@b1ack0u7](https://github.com/b1ack0u7))
- Redraw the download notification only when progress moves, and at most five times a second ([@b1ack0u7](https://github.com/b1ack0u7))
- Restore the download queue on startup with one database query per entry, off the main thread ([@b1ack0u7](https://github.com/b1ack0u7))
- Fetch anime details and episodes in a single request on sources that support it, in parallel otherwise ([@b1ack0u7](https://github.com/b1ack0u7))
- Stop the anime entry screen from making two overlapping requests for details and episodes ([@b1ack0u7](https://github.com/b1ack0u7))
- Stop rewriting chapters and episodes that did not change during a library update ([@b1ack0u7](https://github.com/b1ack0u7))
- Match chapters and episodes against the database by lookup instead of scanning the whole list, so syncing no longer slows down as the count grows ([@b1ack0u7](https://github.com/b1ack0u7))
- Stop the library update from announcing every entry twice to the notification system ([@b1ack0u7](https://github.com/b1ack0u7))

### Fixed

- Fix chapter and episode upload dates being overwritten by the time of the last library refresh ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix new episodes never being queued for download after an anime library update ([@b1ack0u7](https://github.com/b1ack0u7))
- Stop a scheduled anime library update from starting on top of a manual one still running ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix "Library last updated" being stamped when the update started rather than when it finished ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix the status bar and navigation bar being see-through in the reader while the controls were on screen ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix a band of empty space, as tall as the status bar, between the Anime/Manga tabs and the updates list ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix the scrollbar jumping around while scrolling the entry screens, now measured in list items. Ported from Mihon ([@anirudhn](https://github.com/anirudhn)) ([mihon#2304](https://github.com/mihonapp/mihon/pull/2304)) and carried over to the grid variant the anime screen uses ([@b1ack0u7](https://github.com/b1ack0u7))
...
- Fix the app closing itself when opening a recently updated manga source, by bumping kotlinx-coroutines to 1.11.0 to match the extension repository ([@b1ack0u7](https://github.com/b1ack0u7))
- Show an error in the source listing instead of closing the app when an extension is built against a newer library than the app ships, on both sides ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix an episode being marked as downloaded when the download left only an incomplete temporary file ([@b1ack0u7](https://github.com/b1ack0u7))
- Fix the download worker never ending on its own when the connection drops, on both the anime and manga sides ([@b1ack0u7](https://github.com/b1ack0u7))

### Other

- Remove an unused second copy of the episode download finalisation logic ([@b1ack0u7](https://github.com/b1ack0u7))
